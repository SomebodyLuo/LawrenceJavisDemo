...
public class TextView extends View
{
	...
	private Bitmap mHoldBitmap = null;
	...
	public TextView(boolean showTextDrawableBackGround)
	{
		super();
		SetDebugName("TextView");
	...
	private void drawText()
	{
		...
		if(mHoldBitmap == null || mResetBitmapFlag){
			long time = System.currentTimeMillis();
			mHoldBitmap = Bitmap.createBitmap(mTextWidth, mTextHeight, Config.ARGB_8888);
			if(mResetBitmapFlag ){
				mResetBitmapFlag = false;
			}
//			Log.d("debug", "craetebitmap time  =  "+(System.currentTimeMillis()-time  ));
		}
		Bitmap bitmap = mHoldBitmap;
		if (isLineText)
		{
			bitmap = drawTextWithCount(bitmap,mTextWidth, mTextHeight, mText, mTextSize, mTextColor, mTextAlpha, mTextScaleX, mTextTypeface, mTextAlign, mTextRelativeSpacing,
		...
		} else if (isSingleLine)
		{
			if (isMarquee && isDrawMarquee)
			{
				...
				bitmap.eraseColor(Color.TRANSPARENT);// 文字背景色
				if (mMarquee.width < mTextWidth)
				{
					bitmap = drawSinglineText(bitmap,mTextWidth, mTextHeight, mText, mTextSize, mTextColor, mTextAlpha, mTextScaleX, mTextTypeface, mTextAlign, isShadow);
					mNeedUpdate = false;
					if (bitmap != null)
					{
						Texture texture = Director.getInstance().sResourcer.generateTexture(bitmap,false);
						mTextDrawable.setTexture(texture,true);
						return;
					}
				}
				bitmap.eraseColor(Color.TRANSPARENT);// 文字背景色
				...
				bitmap = marquee(bitmap,mMarquee.pixels);
//				bitmap.recycle();
//				Log.d("debug", "mMarquee.pixels =   "+mMarquee.pixels + ",,mMarquee.width  = "+mMarquee.width );
...
//						Director.getInstance().postDelayInvalidate(3000);
//					}
		...
		{
			bitmap = drawText(bitmap,mTextWidth, mTextHeight, mText, mTextSize, mTextColor, mTextAlpha, mTextScaleX, mTextTypeface, mTextAlignment, mTextRelativeSpacing,
...
//			long time = System.currentTimeMillis();
			...
			Texture texture = Director.getInstance().sResourcer.generateTexture(bitmap,false);
//			Log.d("debug", "generatettexute  time  =  "+( System.currentTimeMillis()-time ));
			mTextDrawable.setTexture(texture, mNeedRecycleLast);
		}
	}
	/**
	 * 主动调用一次更新纹理
	...
	public void setWidth(float f)
	{
		super.setWidth(f);
		mTextWidth = (int) f;
		mTextDrawable.setWidth(mTextWidth );
	}
	...
	public void setHeight(float f)
	{
		super.setHeight(f);
		mTextHeight = (int) f;
		mTextDrawable.setHeight(mTextHeight );
	}

	private boolean  mResetBitmapFlag = false;
	private boolean mFakeBold = false;;
	public void resetWidth(float f)
	{
		this.setWidth(f);
		mTextWidth = (int) f;
		mResetBitmapFlag = true;
	}
	...
	public void setTextDrawable(Texture texture)
	{
		this.mTextDrawable .setTexture(texture);
	}
	...
	public void setTextPadding(int left, int top, int right, int bottom)
	{
		...
		mTextDrawable.setWidth(mTextWidth );
		mTextDrawable.setHeight(mTextHeight );
		mTextDrawable.setTranslate((float) (left - right) / 2 , (float) (bottom - top)  / 2, 0);
	}

	private Bitmap drawText(Bitmap textBitmap ,int width, int height, String text, int textSize, int color, int alpha, float textScaleX, Typeface textTypeface, Alignment textAlignment,
...
//				}
	...
	private Bitmap drawTextWithCount(Bitmap textBitmap, int width, int height, String text, int textSize, int color, int alpha, float textScaleX, Typeface textTypeface, Align align,
			float textRelaSpac, float textAbsSpac, int count, boolean isShadow)
	{
//		Bitmap textBitmap = Bitmap.createBitmap(width, height, Config.ARGB_8888);
		textBitmap.eraseColor(Color.TRANSPARENT);// 文字背景色
	...
	private Bitmap drawSinglineText(Bitmap textBitmap, int width, int height, String text, int textSize, int color, int alpha, float textScaleX, Typeface textTypeface, Align align, boolean isShadow)
	{
...
//		Bitmap textBitmap = Bitmap.createBitmap(width, height, Config.ARGB_8888);
	...
	private Bitmap marquee(Bitmap textBitmap, int x)
	{
