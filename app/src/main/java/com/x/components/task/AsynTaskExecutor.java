...
//		MLog.d("aaa","======AsynTaskExecutor()==="+this);
	 ...
	 * @return
	 */
	public boolean startAllWorkThread() {
		
		// 更新线程组中的线程数量

		mIsAllStop = false;
//		for (int i = 0; i < mThreadGroup.size(); i++) {
//			TaskExecuteThread thread = mThreadGroup.get(i);
//			if (thread != null) {
//				if(!thread.isAlive()){
//					mThreadGroup.remove(i);
//					TaskExecuteThread t = new TaskExecuteThread();
//					thread.setName(mThreadName + "#" + i);
//					thread.start();
//					mThreadGroup.add(i, t);
//				};
//			}
//		}	
		
		for (int i = 0; i < mMaxThreadCount ; i++) {
			if(mThreadGroup.size() < mMaxThreadCount){
	 ...
	 * @return
	 */
	public boolean stopAllWorkThread(){

		mIsAllStop = true;
		boolean result = false;
		// 更新线程组中的线程数量

//		MLog.d("aaa",  "===stopAllWorkThread()==="+mThreadGroup.size());
...
//				thread.interrupt();
	 ...
	 */
	@SuppressLint("NewApi") 
	public boolean requestAtFrontOfQueue(AsynTask task) {
		boolean result = false;
	...
	protected class TaskExecuteThread extends Thread{
		...
		public void run() {
			try {
				...
				// 线程执行完毕
				MLog.E( Thread.currentThread().getId() + " InterruptedException ");
				exp.printStackTrace();
				
			}catch(Exception exp) {
				// 线程异常退出
				MLog.E( Thread.currentThread().getId() + " Exception ");
				exp.printStackTrace();
			}catch(Error err) {
				// 线程异常退出
				MLog.E( Thread.currentThread().getId() + " Error ");
				err.printStackTrace();
			}finally{
				disposeThread();
			}
