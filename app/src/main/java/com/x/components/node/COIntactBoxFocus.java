 ...
 */
@SuppressLint("NewApi")
public class COIntactBoxFocus extends MoveBox
{
...
//		float translate_Z = -EngineConstanst.PIX_REFERENCE * (1);// 确保飞框在最顶层
		...
		for (int i = 0; i < mModelMatrix.length; i++)
		{
			...
			flag |= mModelMatrix[i] != tempModelViewMatrix[i];
	...
	protected void focusDraw() {
		...
		onAnimation();
		mBGDrawable.setModelViewMatrix(mModelMatrix);
		mBGDrawable.draw();
	...
	protected void focusOnAnimation() {
		...
		// ,mThickness);
		mFocusViewAnimation.fill(mModelMatrix);
	...
	class FocusViewAnimation
	{
		...
		public void setDetination(float[] finalModelViewMatrix, boolean byView)
		{
			...
			{
				mAnimationHolders[i].setDestinationIndex(finalModelViewMatrix[i], AnimationHolder.RUNNING, speed, true, true);
			}
		}
