 ...
 */
@SuppressLint("NewApi")
public class CopyOfIntactBoxFocus extends MoveBox
{
...
//		float translate_Z = -EngineConstanst.PIX_REFERENCE * (1);// 确保飞框在最顶层
	...
	protected void focusOnAnimation() {
		...
		mHasAnimation = mFocusViewAnimation.hasMoreAnimation();
		if (mHasAnimation)
		{
			postInvalidate();
		}
...
//		mHasAnimation = mFocusViewAnimation.hasMoreAnimation();
	...
	class FocusViewAnimation
	{
		...
		private XScroller[] mwolfScrollers = new XScroller[16]  ;
//		private AnimationHolder[] mAnimationHolders = new AnimationHolder[16];

		public FocusViewAnimation()
		{
			for (int i = 0; i < mwolfScrollers.length; i++)
			{
//				mAnimationHolders[i] = new AnimationHolder(0, 1);

				mwolfScrollers[i] = new XScroller(Director.getInstance().getContext(), 0, 10000);
			}
		}
		...
		public void fill(float[] modelViewMatrix)
		{
			...
			for (int i = 0; i < mwolfScrollers.length; i++)
			{
				modelViewMatrix[i] = mwolfScrollers[i].getCurrentIndex();
			}
		}
		...
		public void setDetination(float[] finalModelViewMatrix, boolean byView)
		{
			...
			{
				if(mwolfScrollers[i].getCurrentIndex() != finalModelViewMatrix[i]){
					
					mwolfScrollers[i].scrollToTargetIndex(finalModelViewMatrix[i], Transform.ANIMATION_DURATION);
				}
//				(finalModelViewMatrix[i], AnimationHolder.RUNNING, speed, true, true);
			}
		}
...
//			}
		...
		public boolean hasMoreAnimation()
		{
			...
			for (int i = 0; i < mwolfScrollers.length; i++)
			{
				has |= mwolfScrollers[i].computeOffset();
//				if (has)
//				{
//					break;
//				}
